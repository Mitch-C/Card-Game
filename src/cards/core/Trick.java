...
public class Trick implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4207403513107799442L;
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		List<Card> ListCards = getCardsPlayed();	
		// Must have the card to be able to play it
		if(!p.hand.contains(c)) {
			throw new IllegalMove("You dont have that card");
		}	
		//Make sure the player has cards
		if(!ListCards.isEmpty()) {
		// If its not your turn throw error
			if(p.direction != getNextToPlay()) {
				throw new IllegalMove("It is not your turn");
			}	
			// if suit doesnt match creat a set that does
			if(c.suit() != getCardPlayed(lead).suit()) {
			
				Set<Card> Matches = p.getHand().matches(getCardPlayed(lead).suit());			
				if((!Matches.isEmpty())) {
					throw new IllegalMove("you must play your trump or card of same suit");
				}
				}
		} else {
			if(p.direction != lead) {
				throw new IllegalMove("You are not the current player");
			}
		}		
